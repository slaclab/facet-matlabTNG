classdef bsagui_mdl < handle
    ...
    methods % EDEF
        ...
        function setupeDef(mdl, HS)
            ...
            mdl.eDefStr = sprintf('%s%s Private %d', prefix, HS, mdl.eDefNumber);
            mdl.have_eDef = 1;
            updateRootNames(mdl);
            notify(mdl, 'eDefOptionsChanged');
        end
    ...
    methods % SETTERS
        ...
        function setSCPAcq(mdl, acqSCP, acqBPMS, acqRF)
             % update model with user change to acquire SCP check box
             mdl.acqSCP = acqSCP;
             mdl.scpNames.getBPMS = acqBPMS;
             mdl.scpNames.getRF = acqRF;
             mdl.scpNames.use(mdl.scpNames.bpmsIdx) = mdl.scpNames.getBPMS .* mdl.scpNames.bpmsIdx(mdl.scpNames.bpmsIdx);
             mdl.scpNames.use(mdl.scpNames.rfIdx) = mdl.scpNames.getRF .* mdl.scpNames.rfIdx(mdl.scpNames.rfIdx);
             updateRootNames(mdl);
        end
...
%             end
    ...
    methods % SAVE/LOAD
        ...
        function saveData(mdl, saveas)
            ...
            [fname, pathName]=util_dataSave(data, 'BSA', ['data-' char(strrep(mdl.eDefStr,' ','-'))] ,  mdl.t_stamp, saveas);
