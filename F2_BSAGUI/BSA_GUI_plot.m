...
bpm250x = mdl.the_matrix(mdl.bpms.etax_id(numetax-1),:);
bpm450x = mdl.the_matrix(mdl.bpms.etax_id(numetax),:);
...
[x_bpm_corf, x_bpm_rms, etaxcorf, parx250, parx450] = deal(zeros(1,lx));
for j=1:lx
    ...
    try
        ...
        [etaxcorf(j)] = corcoef(bpm250x,mdl.the_matrix(mdl.bpms.x_id(j),:));
        a = util_polyFit(bpm250x(use),mdl.the_matrix(mdl.bpms.x_id(j),use),1);
        [a]=a(1,:);
        if a > 2, a = 2; end
        if a < -2, a = -2; end
        b = util_polyFit(bpm450x(use),mdl.the_matrix(mdl.bpms.x_id(j),use),1);
        [b]=b(1,:);
        if b > 2, b = 2; end
        if b < -2, b = -2; end
        [parx250(j)] = a;
...
[y_bpm_corf, y_bpm_rms, etaycorf, pary250, pary450] = deal(zeros(1,ly));
for j=1:ly
    ...
    try
        ...
        [etaycorf(j)] = corcoef(bpm250x,mdl.the_matrix(mdl.bpms.y_id(j),:));
        a = util_polyFit(bpm250x(use),mdl.the_matrix(mdl.bpms.y_id(j),use),1);
        [a]=a(1,:);
        if a > 2, a = 2; end
        if a < -2, a = -2; end
        b = util_polyFit(bpm450x(use),mdl.the_matrix(mdl.bpms.y_id(j),use),1);
        [b]=b(1,:);
        if b > 2, b = 2; end
        if b < -2, b = -2; end
        [pary250(j)] = a;
...
% bpms
if mdl.isSxr
    etaxIndx1 = strcmp(mdl.bpms.name,'BPMS:LTUS:235');
    etaxIndx2 = strcmp(mdl.bpms.name,'BPMS:LTUS:370');
else
    etaxIndx1 = strcmp(mdl.bpms.name,'BPMS:LTUH:250');
    etaxIndx2 = strcmp(mdl.bpms.name,'BPMS:LTUH:450');
end
mdl.bpms.x_dispersion = (1e3 * parx250 * mdl.bpms.etax(etaxIndx1) + 1e3 * parx450 * mdl.bpms.etax(etaxIndx2))/2 ;
mdl.bpms.y_dispersion = (1e3 * pary250 * mdl.bpms.etax(etaxIndx1) + 1e3 * pary450 * mdl.bpms.etax(etaxIndx2))/2 ;
...
function dual_energy_AllZvsA(mdl, xdata)
...
% Hardcoded separation of SXR and HXR dispersive BPMS. Yuck.
etax_id_HXR = mdl.bpms.etax_id(1:5);
etax_id_SXR = mdl.bpms.etax_id([1:3,6:9]);
etay_id_HXR = mdl.bpms.etay_id(1:5);
etay_id_SXR = mdl.bpms.etay_id(6:9);
...
switch mdl.linac
    case 'CU'
        ...
        seconds = xdata / 360;
    case 'SC'
        indx2 = startsWith(mdl.ROOT_NAME, 'TPG:SYS0:1:TSLPID');
        xdata = mdl.the_matrix(indx2,:);
...
ts_idx = 1:2:size(mdl.the_matrix, 2); %get points from the same timeslot
for section = 1:4
    ref_bpm = disp_bpms_idx(section);
    ref_bpm_data = mdl.the_matrix(ref_bpm, ts_idx);
    ...
    for num = 1:10
        for device = 1:length(jitter_idx)
            data = mdl.the_matrix(jitter_idx(device), ts_idx);
        ...
        device_data = mdl.the_matrix(jitter_idx(max_idx(1)),ts_idx);
    ...
    figure
    pie_chart = pie(relative_jitt, name);
    title(['Sources for ' secn(section) ' Energy Jitter (' num2str(abs(stdE)) '%)'])
    for i=1:length(name)
