classdef bsagui_orbitModel < handle
    ...
    methods
        ...
        function orbit_mdl = bsagui_orbitModel(BG_mdl)
        ...
        function loadOrbit(orbit_mdl)
            % initiate load window, and route to correct data intake
            % function
            
            [data, fname, ~] = util_dataLoad('File to load', 0, {'BSA-data*.mat; orbit*.mat'});
            
            if isempty(data)
                return
            end
            
            if startsWith(fname, 'BSA')
                loadFromBSA(orbit_mdl, data);
            elseif startsWith(fname, 'orbit')
                loadFromOrbit(orbit_mdl, data);
            else
                
            end
            notify(orbit_mdl, 'diffStatus');
        ...
        function loadFromBSA(orbit_mdl, BG_mdl)
            ...
            if isfield(BG_mdl, 'facet')
                isFACET = BG_mdl.facet;
            else
                isFACET = any(contains(BG_mdl.ROOT_NAME, 'PATT:SYS1:1:PULSEID'));
            end
            ...
            elseif isFACET
                % create an orbit with all BPMS
                names = BG_mdl.ROOT_NAME;
                pulses = true(1, size(BG_mdl.the_matrix,2));
                scpData = BG_mdl.the_matrix(:, BG_mdl.hasSCP);
                scpPulses = true(1, size(scpData, 2));
                z = orbit_mdl.getZ(BG_mdl, names);
                orbit_mdl.orbits{end + 1} = orbit(scpData, names, scpPulses, time_stamps, z);
                orbit_mdl.orbitNameList{end + 1} = sprintf('BSA-orbit-%s', t_stamp);
                
                % create an orbit without SCP names
                
                orbit_mdl.orbits{end + 1} = orbit(BG_mdl.the_matrix, names, pulses, time_stamps, z);
                orbit_mdl.orbitNameList{end + 1} = sprintf('All Pulses BSA-orbit-%s', t_stamp);
            else
                names = BG_mdl.ROOT_NAME;
                pulses = true(1, size(BG_mdl.the_matrix,2));
                z = orbit_mdl.getZ(BG_mdl, names);
        ...
        function loadFromOrbit(orbit_mdl, dataStruct)
            ...
            if isfield(dataStruct, 'pulsenum')
                pulseStr = sprintf(' Pulse %d', datastruct.pulsenum);
            else
                pulseStr = '';
            end
            orbit_mdl.orbitNameList{end + 1} = sprintf('orbit-%s%s', dataStruct.ts, pulseStr);
            notify(orbit_mdl, 'orbitLoaded');
        end
        ...
        function changeOrbit(orbit_mdl, newOrbit)
            if nargin < 2
                newOrbit = orbit_mdl.orbitNameList{1};
            end
            ...
            orbit_mdl.orbitIdx = find(strcmp(orbit_mdl.orbitNameList, newOrbit));
            orbit_mdl.currentOrbit = orbit_mdl.orbits{orbit_mdl.orbitIdx};
            notify(orbit_mdl, 'diffStatus');
            notify(orbit_mdl, 'newOrbit');
            notify(orbit_mdl, 'plotOrbit');
        ...
        function diffFromCurrent(orbit_mdl)
            orbitA = orbit_mdl.currentOrbit;
            orbitB = orbitA.orbitSlice;
            orbit_mdl.orbits{end+1} = orbitB;
            currentName = orbit_mdl.orbitNameList{orbit_mdl.orbitIdx};
            newName = sprintf('%s Pulse %d', orbit_mdl.orbitNameList{orbit_mdl.orbitIdx}, orbitA.pulsenum);
            if ~any(strcmp(orbit_mdl.orbitNameList, newName))
                orbit_mdl.orbitNameList{end+1} = newName;
            end
            differenceOrbit(orbit_mdl, currentName, newName);
        end
        ...
        function toggleSCPOrbits(mdl, SCPOnly)
            % change between orbit with all pulses, and orbit with only
            % pulses which have all BPMS. This is important for FACET,
            % where the SCP cannot return as many data points as EPICS.
            mdl.SCPOnly = SCPOnly;
            if mdl.SCPOnly
                newOrbit = mdl.orbitNameList{1};
            else
                newOrbit = mdl.orbitNameList{2};
            end
            changeOrbit(mdl, newOrbit);
        end
        
        function changeOrbitName(mdl, orbitName)
            newName = inputdlg('New Orbit Name');
            idx = strcmp(mdl.orbitNameList, orbitName);
            mdl.orbitNameList(idx) = newName;
            notify(mdl, 'orbitLoaded');
        end
    end
