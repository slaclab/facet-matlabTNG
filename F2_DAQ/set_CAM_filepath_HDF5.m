function set_CAM_filepath_HDF5(obj)
...
lcaPut(obj.daq_pvs.HDF5_FileTemplate, ff);
...
for i=1:numel(filePrefix)
    ...
    fp(1:n_el) = pathASCII;
    lcaPut(obj.daq_pvs.HDF5_FilePath{i}, fp);
end

% Check that all paths exists
for i=1:num_cam
    ext = lcaGet(obj.daq_pvs.HDF5_FilePathExists_RBV(i));
    if (~strcmp(ext,'Yes'))
