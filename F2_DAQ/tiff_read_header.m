...
for j=1:IfdLength
    ...
    switch(TagType)
        case 1 %byte
            TagValue=fread(fid,double(TagLength),'uint8=>uint8');
        case 2 % ASCII
            TagValue=fread(fid,double(TagLength),'uint8=>uint8')';
            if(TagValue(end)==0), TagValue=TagValue(1:end-1); end
            TagValue=char(TagValue);
        case 3 % Word
            TagValue=fread(fid,double(TagLength),'uint16=>uint16');
        case 4 % DWord - Uword
            TagValue=fread(fid,double(TagLength),'uint32=>uint32');
        case 5 % Rational (2 dwords, numerator and denominator)
            TagValue=double(fread(fid,double(TagLength*2),'uint32=>uint32'));
            TagValue=TagValue(1:2:end)/TagValue(2:2:end);
        case 6 % An 8-bit (2-byte) signed (twos-complement) integer.
            TagValue=fread(fid,double(TagLength),'int8=>int8');
        case 7 % A 8-bit byte undefined
            TagValue=fread(fid,double(TagLength),'uint8=>uint8');
        case 8 % 16-bit (2-byte) signed (twos-complement) integer.
            TagValue=fread(fid,double(TagLength),'int16=>int16');
        case 9 % A 32 bit(4-byte) signed (twos-complement) integer
            TagValue=fread(fid,double(TagLength),'int32=>int32');
        case 10 % Two SLONGs, numerator/denominator
            TagValue=double(fread(fid,double(TagLength*2),'int32=>int32'));
            TagValue=TagValue(1:2:end)/TagValue(2:2:end);
        case 11 % Single precision (4-byte) IEEE format
            TagValue=fread(fid,double(TagLength),'single=>single');
        case 12 % Double precision (8-byte) IEEE format
            TagValue=fread(fid,double(TagLength),'double=>double');
        case 13
            TagValue=fread(fid,double(TagLength),'uint32=>uint32');
...
for i=1:length(TiffInfo.StripOffsets);
    ...
    data=fread(fid, double(TiffInfo.StripByteCount(i)/nbyte),type);
