classdef F2_S20WaistScanApp < handle & F2_common
  %F2_S20WaistScanApp Applicaton class for FACET-II Sector 20 IP Configurator
  %
...
%     Q_BMAX = [256 446 457 440 440 440 239 386 240] ; % absolute max field of quads
  ...
  methods
    function obj = F2_S20WaistScanApp(guihan)
      %F2_S20WaistScanApp F2_S20Config application support or S20 Config Watcher
      %
      %C=F2_S20WaistScanApp(guihan)
      % Construct with F2_S20Config application object
      %C=F2_S20WaistScanApp()
...
%         obj.guihan.ListBox.Value = obj.WaistDesName ; % need to deal with waist pointer text
    ...
    function writeBS(obj)
        ...
        try        
            ...
            for idx = 1:length(obj.Q_Match)-3
                fprintf(fileId, '%f ', obj.Q_Match(idx));    
            end
            
            fprintf(fileId, '%f ', obj.guihan.BMATCH_Q0D.Value);
            fprintf(fileId, '%f ', obj.guihan.BMATCH_Q1D.Value);
            fprintf(fileId, '%f', obj.guihan.BMATCH_Q2D.Value);
        catch
    ...
    function PrintFinish(obj)
        newLine= 'Wasit Scan List has been generated!'
        currentText = obj.guihan.Log.Value;
        % Check if the currentText is empty or not
        if isempty(currentText)
            % If empty, initialize as a cell array
            updatedText = {newLine};
        else
            % If not empty, append the new line to the existing text
            updatedText = [currentText; {newLine}]; % Concatenate the new line
        end
    ...
    function UpdateLog(obj)
        ...
        % Initialize position to the end of file
        fseek(fileId, 0, 'eof');
        pos = ftell(fileId);
        foundNewLine = false;

        % Move backwards until a newline or the start of the file is reached
        while pos > 1
            % Move back one byte at a time
            pos = pos - 1;
            ...
            % Check for newline character (depends on OS, here we check for LF)
            if ch == 10  % ASCII code for LF (newline)
                foundNewLine = true;
                break;
            end
        end

        % Read the last line. If no newline was found and we are at the start, read from start.
        if foundNewLine
            fseek(fileId, pos + 1, 'bof');  % Move past the newline
        else
            fseek(fileId, 0, 'bof');  % Go to the start of the file
        end
