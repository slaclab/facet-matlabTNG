function takeLaserTimerData(mTimer,~)
    ...
    for n=1:length(UserData.camerapvs)
        ...
        warning(['Camera array rate is 0 Hz or NaN - no data acquired for ',UserData.camerapvs{n}])
        continue
        end
       ...
       if zeroVals(1)>2*UserData.nDataPointsPerShot
         if str2num(datestr(t(end),'HH'))~=str2num(datestr(t(end-1),'HH'))
            imgData = profmon_grab(UserData.camerapvs{n},0); 
            %img = imgData.img;% Save the raw image
            opts = struct('usemethod',2);
            [img,~,~,~,~]=beamAnalysis_imgProc(imgData,opts);% Processed Image
            ...
            % Print one image to logbook @ 8AM and 8PM
            if n==length(UserData.camerapvs) 
                if ~isempty(regexp(datestr(t(end),'HH'),'08')) || ~isempty(regexp(datestr(t(end),'HH'),'20'))
                    ...
                    util_printLog_wComments(55,'autoS10LaserImageDump','Matlab',char(commentString{:}));close(55);
                    catch
                        disp('Could not auto dump laser images to logbook')
                    figure(55);util_printLog_wComments(55,'autoS10LaserImageDump','Matlab','Error auto dumping laser images to logbook');close(55);    
                    end
                end
             % Check for catastrophic laser event once/hour (i.e. laser down on IR cams)
              checkForLaserDrift(UserData);
            end
         end
       end
