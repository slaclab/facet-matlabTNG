classdef F2_S20Config_exported < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        CalcMatchButton                 matlab.ui.control.Button
  ...
  properties (Access = private)
    ...
    methods (Access = private)
      ...
      switch string(value)
        ...
        % Button pushed function: CalcMatchButton
        function CalcMatchButtonPushed(app, event)
      app.XYSelect=false;
      app.CalcMatchButton.Enable=false; drawnow;
      try
        app.aobj.MatchFFS;
      catch ME
        errordlg('Error when running matching routine- see console output','Match Error');
        app.CalcMatchButton.Enable=true;
        throw(ME);
      end
      app.CalcMatchButton.Enable=true;
        end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create CalcMatchButton
            app.CalcMatchButton = uibutton(app.MatchingPanel, 'push');
            app.CalcMatchButton.ButtonPushedFcn = createCallbackFcn(app, @CalcMatchButtonPushed, true);
            app.CalcMatchButton.Interruptible = 'off';
            app.CalcMatchButton.BackgroundColor = [0 1 0];
            app.CalcMatchButton.FontSize = 16;
            app.CalcMatchButton.FontWeight = 'bold';
            app.CalcMatchButton.Tooltip = {'Calculates match only, does not change magnets.'};
            app.CalcMatchButton.Position = [519 296 118 26];
            app.CalcMatchButton.Text = 'Calc. Match';
            ...
            app.TRIMQUADSButton.BackgroundColor = [0 0 0];
            app.TRIMQUADSButton.FontSize = 16;
            app.TRIMQUADSButton.FontWeight = 'bold';
            app.TRIMQUADSButton.FontColor = [1 1 1];
            app.TRIMQUADSButton.Enable = 'off';
            app.TRIMQUADSButton.Tooltip = {'Trims quads to selected match. Actually changes magnets!'};
